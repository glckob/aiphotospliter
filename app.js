...
const translations = {
    ...
    editDescLabel: { km: "Post Description:", en: "Description:" },
    ...
    // Dynamic JS Text (Errors, Status, Buttons)
    ...
    adDescriptionLabel: { km: "ខ្លឹមសារ៖", en: "Description:" }
};
...
function setLanguage(lang) {
    ...
    // RENDER ADS AGAIN to update description labels on language change
...
function renderAds(ads) {
    ...
    adLeftContainer.className = 'p-3 border border-gray-200 rounded-lg bg-gray-50'; // New container
    
    const imgLeft = document.createElement('img');
    imgLeft.src = adLeft.post_img_url;
    imgLeft.alt = "Commercial Ad";
    imgLeft.className = "w-full h-auto object-cover cursor-pointer hover:opacity-90 transition-opacity rounded-md mb-2";
    imgLeft.onclick = () => openAdModal(adLeft);
    adLeftContainer.appendChild(imgLeft);

    const descLeft = document.createElement('p');
    descLeft.className = "text-sm text-gray-700 font-siemreap";
    descLeft.innerHTML = `<span class="font-bold">${translations.adDescriptionLabel[currentLanguage]} </span>${adLeft.post_description}`;
    leftContent.appendChild(adLeftContainer); // Append the container
    leftWrapper.classList.remove('hidden');

    // Render Right Ads (All)
    ads.forEach(ad => {
        const adRightContainer = document.createElement('div');
        adRightContainer.className = 'p-3 border border-gray-200 rounded-lg bg-white shadow-sm';
        
        const imgRight = document.createElement('img');
        imgRight.src = ad.post_img_url;
        imgRight.alt = "Commercial Ad";
        imgRight.className = "w-full h-auto rounded-md object-cover cursor-pointer hover:opacity-90 transition-opacity mb-2";
        imgRight.onclick = () => openAdModal(ad);
        adRightContainer.appendChild(imgRight);
        
        const descRight = document.createElement('p');
        descRight.className = "text-sm text-gray-700 font-siemreap truncate";
        descRight.innerHTML = `<span class="font-bold">${translations.adDescriptionLabel[currentLanguage]} </span>${ad.post_description}`;
        adRightContainer.appendChild(descRight);

        rightContent.appendChild(adRightContainer);
    });
}
...
function renderAdminAdsList(ads) {
    ...
    if (ads.length === 0) {
        list.innerHTML = `<p class="text-gray-500 text-center">${translations.adminNoAds[currentLanguage]}</p>`;
        return;
    }
    ...
    ads.forEach(ad => {
        ...
        item.innerHTML = `
            ...
            <div class="flex gap-2 shrink-0">
                <button class="bg-yellow-500 text-white text-xs px-2 py-1 rounded edit-btn">${translations.adminEdit[currentLanguage]}</button>
                <button class="bg-red-500 text-white text-xs px-2 py-1 rounded delete-btn">${translations.adminDelete[currentLanguage]}</button>
            </div>
        `;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('ad-modal-close').onclick = () => showModal('ad-content-modal', false); // FIX 1
...
async function createFaceCard(box, index, padYPercent, padXPercent) {
    ...
    // Sliders (Pad Y, X, Move Y, X)
    const createSlider = (key, val, min, max) => {
        const div = document.createElement('div');
        div.className = "text-xs";
        // FIX 2: Use translations object safely
        const label = translations[key] ? translations[key][currentLanguage] : key; 
    ...
    const sPadY = createSlider('padYLabel', padYPercent, 0, 100);
    const sPadX = createSlider('padXLabel', padXPercent, 0, 100);
    const sMovY = createSlider('moveYLabel', 0, -100, 100);
    const sMovX = createSlider('moveXLabel', 0, -100, 100);
